...
/**
 * nfp_ual_get_nicmod_count() - return the number of NICmods probed
 *
 * @bitmap:	if non NULL, return bitmap of the NICmods present
 *
 * Return: NICmod count or negative error
 */
int nfp_ual_get_nicmod_count(u8 *bitmap)
{
	...
	for (i = 0; i < NFP_MBL_DEV_ID_MAX; i++) {
		dev_index = NFP_MBL_DEV_INDEX(NFP_MBL_DEV_TYPE_NICMOD, i);
		if (ctx->dev_ctx[dev_index]) {
			count++;
			map |= 1 << i;
		}
