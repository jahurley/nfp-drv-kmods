...
{
	return nfp_pf_rtsym_read_optional(pf, "nfd_cfg_pf%u_num_ports", 0);
}
...
static int nfp_net_pci_map_mem(struct nfp_pf *pf)
{
	...
	mem = nfp_cpp_map_area(pf->cpp, "net.qc", cpp_id,
			       nfp_pci_queue(pf->pdev, 0),
			       nfp_qcp_queue_area_sz(pf->pdev), &pf->qc_area);
	if (IS_ERR(mem)) {
			...
			struct nfp_eth_table *eth_table)
{
	...
	if (eth_port->speed != port->eth_port->speed)
		nfp_app_eth_port_speed_changed(port->app, port, eth_port);
